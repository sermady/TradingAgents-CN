...
def init_logging(config_override: Optional[dict] = None) -> None:
    ...
    # 记录初始化信息
    logger.info("[START] TradingAgents-CN 日志系统初始化完成")
    logger.info(f"[FOLDER] 日志目录: {logger_manager.config.get('handlers', {}).get('file', {}).get('directory', 'N/A')}")
    logger.info(f"[CHART] 日志级别: {logger_manager.config.get('level', 'INFO')}")
    ...
    # 记录环境信息
    logger.debug(f"[CONFIG] Python版本: {sys.version}")
    logger.debug(f"[FOLDER-OPEN] 工作目录: {os.getcwd()}")
    logger.debug(f"[ENV] 环境变量: DOCKER_CONTAINER={os.getenv('DOCKER_CONTAINER', 'false')}")
...
def log_startup_info():
    ...
    logger.info("=" * 60)
    logger.info("[TARGET] TradingAgents-CN 启动")
    logger.info("=" * 60)
    
    # 系统信息
    import platform
    logger.info(f"[MONITOR]  系统: {platform.system()} {platform.release()}")
...
def log_shutdown_info():
    ...
    logger.info("=" * 60)
    logger.info("[STOP] TradingAgents-CN 关闭")
    logger.info("=" * 60)
