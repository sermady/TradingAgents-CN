...
def create_neutral_debator(llm):
    def neutral_node(state) -> dict:
        ...
        # [CHART] 记录所有输入数据的长度，用于性能分析
        logger.info(f"[CHART] [Neutral Analyst] 输入数据长度统计:")
        ...
        logger.info(f"  - [ALERT] 总Prompt长度: {total_prompt_length:,} 字符 (~{total_prompt_length//4:,} tokens)")
...
通过批判性地分析双方来积极参与，解决激进和保守论点中的弱点，倡导更平衡的方法。挑战他们的每个观点，说明为什么适度风险策略可能提供两全其美的效果，既提供增长潜力又防范极端波动。专注于辩论而不是简单地呈现数据，旨在表明平衡的观点可以带来最可靠的结果。请用中文以对话方式输出，就像您在说话一样，不使用任何特殊格式。"""
        ...
        logger.info(f"[TIME] [Neutral Analyst] 开始调用LLM...")
        llm_start_time = time.time()

        response = llm.invoke(prompt)

        llm_elapsed = time.time() - llm_start_time
        logger.info(f"[TIME] [Neutral Analyst] LLM调用完成，耗时: {llm_elapsed:.2f}秒")
        logger.info(f"[LOG] [Neutral Analyst] 响应长度: {len(response.content):,} 字符")
