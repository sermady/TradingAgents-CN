...
class ConfigService:
    ...
    async def update_datasource_grouping(self, data_source_name: str, category_id: str, updates: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # 2. [HOT] 如果更新了优先级，同步更新 system_configs 集合
                ...
                if config_data:
                    ...
                    for ds_config in data_source_configs:
                        ...
                        if (ds_config.get("name") == data_source_name or
                            ...
                            updated = True
                            logger.info(f"[OK] [优先级同步] 更新 system_configs 中的数据源: {data_source_name}, 新优先级: {updates['priority']}")
                            break
                    ...
                    if updated:
                        ...
                        logger.info(f"[OK] [优先级同步] system_configs 版本更新: {version} -> {version + 1}")
                    else:
                        logger.warning(f"[WARN] [优先级同步] 未找到匹配的数据源配置: {data_source_name}")

            return result.modified_count > 0
        except Exception as e:
            logger.error(f"[FAIL] 更新数据源分组关系失败: {e}")
            return False
    ...
    async def update_category_datasource_order(self, category_id: str, ordered_datasources: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            # 2. [HOT] 同步更新 system_configs 集合中的 data_source_configs
            # 获取当前激活的配置
            config_data = await config_collection.find_one(
                {"is_active": True},
                sort=[("version", -1)]
            ...
            if config_data:
                ...
                for ds_config in data_source_configs:
                    ...
                    if ds_name in priority_map:
                        ...
                        updated = True
                        print(f"[CHART] [优先级同步] 更新数据源 {ds_name} 的优先级为 {priority_map[ds_name]}")
                ...
                if updated:
                    ...
                    print(f"[OK] [优先级同步] 已同步更新 system_configs 集合，新版本: {config_data.get('version', 0) + 1}")
                else:
                    print(f"[WARN] [优先级同步] 没有找到需要更新的数据源配置")
            else:
                print(f"[WARN] [优先级同步] 未找到激活的系统配置")

            return True
        except Exception as e:
            print(f"[FAIL] 更新分类数据源排序失败: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    async def get_system_config(self) -> Optional[SystemConfig]:
        ...
        try:
            ...
            if config_data:
                print(f"[CHART] 从数据库获取配置，版本: {config_data.get('version', 0)}, LLM配置数量: {len(config_data.get('llm_configs', []))}")
                return SystemConfig(**config_data)

            # 如果没有配置，创建默认配置
            print("[WARN] 数据库中没有配置，创建默认配置")
            return await self._create_default_config()

        except Exception as e:
            print(f"[FAIL] 从数据库获取配置失败: {e}")

            # 作为最后的回退，尝试从统一配置管理器获取
            try:
                unified_system_config = await unified_config.get_unified_system_config()
                if unified_system_config:
                    print("[SYNC] 回退到统一配置管理器")
    ...
    async def save_system_config(self, config: SystemConfig) -> bool:
        """保存系统配置到数据库"""
        try:
            print(f"[SAVE] 开始保存配置，LLM配置数量: {len(config.llm_configs)}")
            ...
            print(f"[LOG] 禁用旧配置数量: {update_result.modified_count}")
    ...
    async def delete_llm_config(self, provider: str, model_name: str) -> bool:
        ...
        try:
            ...
            print(f"[CHART] 当前大模型配置数量: {len(config.llm_configs)}")
            ...
            print(f"[SYNC] 删除后配置数量: {new_count} (原来: {original_count})")

            if new_count == original_count:
                print(f"[FAIL] 没有找到匹配的配置: {provider}/{model_name}")
                return False  # 没有找到要删除的配置

            # 保存更新后的配置
            save_result = await self.save_system_config(config)
            print(f"[SAVE] 保存结果: {save_result}")

            return save_result

        except Exception as e:
            print(f"[FAIL] 删除LLM配置失败: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    async def update_system_settings(self, settings: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # 打印更新前的系统设置
            print(f"[LOG] 更新前 system_settings 包含 {len(config.system_settings)} 项")
            if 'quick_analysis_model' in config.system_settings:
                ...
                print(f"  [WARN]  更新前不包含 quick_analysis_model")

            # 更新系统设置
            config.system_settings.update(settings)

            # 打印更新后的系统设置
            print(f"[LOG] 更新后 system_settings 包含 {len(config.system_settings)} 项")
            if 'quick_analysis_model' in config.system_settings:
                print(f"  ✓ 更新后包含 quick_analysis_model: {config.system_settings['quick_analysis_model']}")
            else:
                print(f"  [WARN]  更新后不包含 quick_analysis_model")
            if 'deep_analysis_model' in config.system_settings:
                ...
                print(f"  [WARN]  更新后不包含 deep_analysis_model")
            ...
            if result:
                try:
                    from app.core.unified_config import unified_config
                    unified_config.sync_to_legacy_format(config)
                    print(f"[OK] 系统设置已同步到文件系统")
                except Exception as e:
                    print(f"[WARN]  同步系统设置到文件系统失败: {e}")
    ...
    async def test_llm_config(self, llm_config: LLMConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"[LOC] API基础URL (模型配置): {llm_config.api_base}")
            ...
            if not api_base:
                ...
                if provider_data and provider_data.get("default_base_url"):
                    ...
                    logger.info(f"[OK] 从厂家配置获取 API 基础 URL: {api_base}")
                else:
                    return {
                        "success": False,
            ...
            if llm_config.api_key:
                ...
                if provider_data and provider_data.get("api_key"):
                    ...
                    logger.info(f"[OK] 从厂家配置获取到API密钥")
                else:
                    # 尝试从环境变量获取
                    api_key = self._get_env_api_key(provider_str)
                    if api_key:
                        logger.info(f"[OK] 从环境变量获取到API密钥")
            ...
            if provider_str == "google":
                # Google AI 使用专门的测试方法
                logger.info(f"[SEARCH] 使用 Google AI 专用测试方法")
                ...
                # DeepSeek 使用专门的测试方法
                logger.info(f"[SEARCH] 使用 DeepSeek 专用测试方法")
                ...
                logger.info(f"[WEB] 发送测试请求到: {url}")
                logger.info(f"[PKG] 使用模型: {llm_config.model_name}")
                logger.info(f"[PKG] 请求数据: {data}")
                ...
                if response.status_code == 200:
                    try:
                        ...
                        logger.info(f"[PKG] 响应JSON: {result}")

                        if "choices" in result and len(result["choices"]) > 0:
                            content = result["choices"][0]["message"]["content"]
                            logger.info(f"[LOG] 响应内容: {content}")

                            if content and len(content.strip()) > 0:
                                logger.info(f"[OK] 测试成功: {content[:50]}")
                                return {
                                    "success": True,
                                ...
                                logger.warning(f"[WARN] API响应内容为空")
                                return {
                                    "success": False,
                                    "message": "API响应内容为空",
                                    "response_time": response_time,
                            ...
                            logger.warning(f"[WARN] API响应格式异常，缺少 choices 字段")
            ...
            response_time = time.time() - start_time
            logger.error(f"[FAIL] 测试大模型配置失败: {e}")
            return {
                "success": False,
    ...
    async def test_data_source_config(self, ds_config: DataSourceConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            # [HOT] 优先使用配置中的 API Key，如果没有或被截断，则从数据库获取
            api_key = ds_config.api_key
            used_db_credentials = False
            used_env_credentials = False

            logger.info(f"[SEARCH] [TEST] Received API Key from config: {repr(api_key)} (type: {type(api_key).__name__}, length: {len(api_key) if api_key else 0})")

            # 根据不同的数据源类型进行测试
            if ds_type == "tushare":
                # [HOT] 如果配置中的 API Key 包含 "..."（截断标记），需要验证是否是未修改的原值
                if api_key and "..." in api_key:
                    logger.info(f"[SEARCH] [TEST] API Key contains '...' (truncated), checking if it matches database value")
                    ...
                    if db_config and db_config.api_key:
                        ...
                        logger.info(f"[SEARCH] [TEST] Database API Key truncated: {truncated_db_key}")
                        logger.info(f"[SEARCH] [TEST] Received API Key: {api_key}")
                        ...
                        if api_key == truncated_db_key:
                            ...
                            logger.info(f"[OK] [TEST] Truncated values match, using complete API Key from database (length: {len(api_key)})")
                        else:
                            # 不同，说明用户修改了但修改得不完整
                            logger.error(f"[FAIL] [TEST] Truncated API Key doesn't match database value, user may have modified it incorrectly")
                            return {
                                "success": False,
                        ...
                        logger.info(f"[WARN]  [TEST] No valid API Key in database, trying environment variable")
                        ...
                        if env_token:
                            ...
                            logger.error(f"[FAIL] [TEST] No valid API Key in database or environment")
                            return {
                                "success": False,
                ...
                # 如果 API Key 为空，尝试从数据库或环境变量获取
                    ...
                    logger.info(f"[WARN]  [TEST] API Key is empty, trying to get from database")
                    ...
                    if db_config and db_config.api_key and "..." not in db_config.api_key:
                        ...
                        if env_token:
                            ...
                            logger.error(f"[FAIL] [TEST] No valid API Key in config, database, or environment")
                            return {
                                "success": False,
                    ...
                    # API Key 是完整的，直接使用
                    logger.info(f"[OK] [TEST] Using complete API Key from config (length: {len(api_key)})")

                # 测试 Tushare API
                try:
                    logger.info(f"[PORT] [TEST] Calling Tushare API with token (length: {len(api_key)})")
                    ...
                    if df is not None and len(df) > 0:
                        response_time = time.time() - start_time
                        logger.info(f"[OK] [TEST] Tushare API call successful (response time: {response_time:.2f}s)")
                        ...
                        logger.error(f"[FAIL] [TEST] Tushare API returned empty data")
                        return {
                            "success": False,
                    ...
                    logger.error(f"[FAIL] [TEST] Tushare library not installed")
                    return {
                        "success": False,
                    ...
                    logger.error(f"[FAIL] [TEST] Tushare API call failed: {e}")
                    return {
                        "success": False,
                ...
                if api_key and "..." in api_key:
                    ...
                    if db_config and db_config.api_key:
                        ...
                        if api_key == truncated_db_key:
                            ...
                            # 不同，说明用户修改了但修改得不完整
                            logger.error(f"[FAIL] [TEST] Truncated API Key doesn't match database value")
                            return {
                                "success": False,
                                "message": "API Key 格式错误：检测到截断标记但与数据库中的值不匹配，请输入完整的 API Key",
                                "response_time": time.time() - start_time,
                ...
                try:
                    logger.info(f"[PORT] [TEST] Calling Alpha Vantage API with key (length: {len(api_key)})")
                    response = requests.get(url, params=params, timeout=10)
                    ...
                    if response.status_code == 200:
                        ...
                        if "Time Series (5min)" in data or "Meta Data" in data:
                            response_time = time.time() - start_time
                            logger.info(f"[OK] [TEST] Alpha Vantage API call successful (response time: {response_time:.2f}s)")
            ...
            response_time = time.time() - start_time
            logger.error(f"[FAIL] 测试数据源配置失败: {e}")
            return {
                "success": False,
                "message": f"连接失败: {str(e)}",
                "response_time": response_time,
    ...
    async def test_database_config(self, db_config: DatabaseConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"[LOC] 连接地址: {db_config.host}:{db_config.port}")
            ...
            if db_type == "mongodb":
                try:
                    ...
                    # [HOT] 优先使用环境变量中的完整连接信息（包括host、用户名、密码）
                    host = db_config.host
                    port = db_config.port
                    ...
                    if not username or not password:
                        ...
                        if env_username and env_password:
                            ...
                            if env_host:
                                host = env_host
                                # [HOT] Docker 环境下，将 localhost 替换为 mongodb
                    ...
                    if not database:
                        ...
                        if env_database:
                            ...
                            logger.info(f"[PKG] 使用环境变量中的数据库名: {database}")
                    ...
                    logger.info(f"[LINK] 连接字符串: {connection_string.replace(password or '', '***') if password else connection_string}")
                    ...
                    error_msg = str(e)
                    logger.error(f"[FAIL] MongoDB 连接测试失败: {error_msg}")
                ...
                try:
                    ...
                    # [HOT] 优先使用环境变量中的完整 Redis 配置（包括host、密码）
                    host = db_config.host
                    port = db_config.port
                    password = db_config.password
                    database = db_config.database
                    ...
                    if not password:
                        ...
                        if env_password:
                            ...
                            if env_host:
                                host = env_host
                                # [HOT] Docker 环境下，将 localhost 替换为 redis
                                if is_docker and host == 'localhost':
                                    host = 'redis'
                    ...
                    if database is None:
                        ...
                        if env_db:
                            database = int(env_db)
                            logger.info(f"[PKG] 使用环境变量中的 Redis 数据库编号: {database}")
    ...
    async def add_database_config(self, db_config: DatabaseConfig) -> bool:
        ...
        try:
            ...
            result = await self.save_system_config(config)
            if result:
                logger.info(f"[OK] 数据库配置 '{db_config.name}' 添加成功")
            else:
                logger.error(f"[FAIL] 数据库配置 '{db_config.name}' 添加失败")
    ...
    async def update_database_config(self, db_config: DatabaseConfig) -> bool:
        """更新数据库配置"""
        try:
            logger.info(f"[SYNC] 更新数据库配置: {db_config.name}")
            ...
            if not found:
                logger.error(f"[FAIL] 数据库配置 '{db_config.name}' 不存在")
                return False

            # 保存配置
            result = await self.save_system_config(config)
            if result:
                logger.info(f"[OK] 数据库配置 '{db_config.name}' 更新成功")
            else:
                logger.error(f"[FAIL] 数据库配置 '{db_config.name}' 更新失败")
    ...
    async def delete_database_config(self, db_name: str) -> bool:
        ...
        try:
            ...
            if new_count == original_count:
                logger.error(f"[FAIL] 数据库配置 '{db_name}' 不存在")
                return False

            # 保存配置
            result = await self.save_system_config(config)
            if result:
                logger.info(f"[OK] 数据库配置 '{db_name}' 删除成功")
            else:
                logger.error(f"[FAIL] 数据库配置 '{db_name}' 删除失败")
    ...
    async def get_database_configs(self) -> List[DatabaseConfig]:
        ...
        try:
            ...
            logger.error(f"[FAIL] 获取数据库配置列表失败: {e}")
            return []
    ...
    async def init_default_model_catalog(self) -> bool:
        ...
        try:
            ...
            print(f"[OK] 初始化了 {len(default_catalogs)} 个厂家的模型目录")
            return True
        except Exception as e:
            print(f"初始化模型目录失败: {e}")
            return False
    ...
    async def get_llm_providers(self) -> List[LLMProvider]:
        ...
        try:
            ...
            logger.info(f"[SEARCH] [get_llm_providers] 从数据库获取到 {len(providers_data)} 个供应商")

            for provider_data in providers_data:
                provider = LLMProvider(**provider_data)

                # [HOT] 判断数据库中的 API Key 是否有效
                db_key_valid = self._is_valid_api_key(provider.api_key)
                logger.info(f"[SEARCH] [get_llm_providers] 供应商 {provider.display_name} ({provider.name}): 数据库密钥有效={db_key_valid}")
                ...
                if not db_key_valid:
                    ...
                    logger.info(f"[SEARCH] [get_llm_providers] 尝试从环境变量获取 {provider.name} 的 API 密钥...")
                    ...
                    if env_key:
                        ...
                        logger.info(f"[OK] [get_llm_providers] 从环境变量为厂家 {provider.display_name} 获取API密钥")
                    else:
                        provider.extra_config["has_api_key"] = False
                        logger.warning(f"[WARN] [get_llm_providers] 厂家 {provider.display_name} 的数据库配置和环境变量都未配置有效的API密钥")
                else:
                    # 数据库中的 Key 有效，使用数据库配置
                    provider.extra_config["source"] = "database"
                    provider.extra_config["has_api_key"] = True
                    logger.info(f"[OK] [get_llm_providers] 使用数据库配置的 {provider.display_name} API密钥")

                providers.append(provider)

            logger.info(f"[SEARCH] [get_llm_providers] 返回 {len(providers)} 个供应商")
            return providers
        except Exception as e:
            logger.error(f"[FAIL] [get_llm_providers] 获取厂家列表失败: {e}", exc_info=True)
            return []
    ...
    async def delete_llm_provider(self, provider_id: str) -> bool:
        ...
        try:
            ...
            print(f"[SEARCH] ObjectId类型: {type(ObjectId(provider_id))}")

            db = await self._get_db()
            providers_collection = db.llm_providers
            print(f"[CHART] 数据库: {db.name}, 集合: {providers_collection.name}")

            # 先列出所有厂家的ID，看看格式
            all_providers = await providers_collection.find({}, {"_id": 1, "display_name": 1}).to_list(length=None)
            print(f"[CLIPBOARD] 数据库中所有厂家ID:")
            ...
            # 尝试不同的查找方式
            print(f"[SEARCH] 尝试用ObjectId查找...")
            existing1 = await providers_collection.find_one({"_id": ObjectId(provider_id)})

            print(f"[SEARCH] 尝试用字符串查找...")
            existing2 = await providers_collection.find_one({"_id": provider_id})

            print(f"[SEARCH] ObjectId查找结果: {existing1 is not None}")
            print(f"[SEARCH] 字符串查找结果: {existing2 is not None}")

            existing = existing1 or existing2
            if not existing:
                print(f"[FAIL] 两种方式都找不到厂家: {provider_id}")
                return False

            print(f"[OK] 找到厂家: {existing.get('display_name')}")
            ...
            for provider_name, config in AGGREGATOR_PROVIDERS.items():
                ...
                if existing:
                    ...
                    if not existing.get("api_key") and api_key:
                        ...
                        updated_count += 1
                        print(f"[OK] 更新聚合渠道 {config['display_name']} 的 API Key")
                    else:
                        skipped_count += 1
                ...
                if api_key:
                    print(f"[OK] 添加聚合渠道: {config['display_name']} (已从环境变量获取 API Key)")
                else:
                    print(f"[OK] 添加聚合渠道: {config['display_name']} (需手动配置 API Key)")
            ...
            for provider_config in default_providers:
                ...
                if existing:
                    ...
                    if not existing.get("api_key") and api_key:
                        ...
                        updated_count += 1
                        print(f"[OK] 更新厂家 {provider_config['display_name']} 的API密钥")
                    else:
                        skipped_count += 1
                ...
                migrated_count += 1
                print(f"[OK] 创建厂家 {provider_config['display_name']}")
    ...
    async def test_provider_api(self, provider_id: str) -> dict:
        """测试厂家API密钥"""
        try:
            print(f"[SEARCH] 测试厂家API - provider_id: {provider_id}")
            ...
            if not self._is_valid_api_key(api_key):
                ...
                if env_api_key:
                    api_key = env_api_key
                    print(f"[OK] 数据库配置无效，从环境变量读取到 {display_name} 的 API Key")
                else:
                    return {
                        "success": False,
                ...
                print(f"[OK] 使用数据库配置的 {display_name} API密钥")
    ...
    async def _test_provider_connection(self, provider_name: str, api_key: str, display_name: str) -> dict:
        ...
        try:
            ...
            if provider_name in ["302ai", "oneapi", "newapi", "custom_aggregator"]:
                ...
                # [CONFIG] 对于未知的自定义厂家，使用 OpenAI 兼容 API 测试
                logger.info(f"[SEARCH] 使用 OpenAI 兼容 API 测试自定义厂家: {provider_name}")
                # 获取厂家的 base_url
                db = await self._get_db()
                providers_collection = db.llm_providers
                provider_data = await providers_collection.find_one({"name": provider_name})
    ...
    def _test_google_api(self, api_key: str, display_name: str, base_url: str = None, model_name: str = None) -> dict:
        ...
        try:
            ...
            if not model_name:
                ...
                logger.info(f"[WARN] 未指定模型，使用默认模型: {model_name}")

            logger.info(f"[SEARCH] [Google AI 测试] 开始测试")
            ...
            if not base_url:
                ...
                logger.info(f"   [WARN] base_url 为空，使用默认值: {base_url}")
            ...
            if base_url.endswith('/v1'):
                ...
                logger.info(f"   [OK] 将 /v1 替换为 /v1beta: {base_url}")

            # 构建完整的 API 端点（使用用户配置的模型）
            url = f"{base_url}/models/{model_name}:generateContent?key={api_key}"

            logger.info(f"[LINK] [Google AI 测试] 最终请求 URL: {url.replace(api_key, '***')}")

            headers = {
                "Content-Type": "application/json"
            }

            # [CONFIG] 增加 token 限制到 2000，避免思考模式消耗导致无输出
            data = {
                "contents": [{
                    "parts": [{
            ...
            print(f"[IMPORT] [Google AI 测试] 响应状态码: {response.status_code}")

            if response.status_code == 200:
                # 打印完整的响应内容用于调试
                print(f"[IMPORT] [Google AI 测试] 响应内容（前1000字符）: {response.text[:1000]}")

                result = response.json()
                print(f"[IMPORT] [Google AI 测试] 解析后的 JSON 结构:")
                ...
                if "candidates" in result and len(result["candidates"]) > 0:
                    ...
                    print(f"[IMPORT] [Google AI 测试] candidate 结构: {candidate}")

                    # 检查 finishReason
                    finish_reason = candidate.get("finishReason", "")
                    print(f"[IMPORT] [Google AI 测试] finishReason: {finish_reason}")
                    ...
                    if "content" in candidate:
                        ...
                        if "parts" in content and len(content["parts"]) > 0:
                            ...
                            print(f"[IMPORT] [Google AI 测试] 提取的文本: {text}")
                            ...
                            if text and len(text.strip()) > 0:
                                ...
                                print(f"[FAIL] [Google AI 测试] 文本为空")
                                return {
                                    "success": False,
                            ...
                            print(f"[FAIL] [Google AI 测试] content 中没有 parts")
                        ...
                        print(f"[FAIL] [Google AI 测试] candidate 中缺少 'content'")
                    ...
                    print(f"[FAIL] [Google AI 测试] 缺少 candidates 或 candidates 为空")
                    return {
                        "success": False,
                        "message": f"{display_name} API无有效候选响应"
                    }
            elif response.status_code == 400:
                print(f"[FAIL] [Google AI 测试] 400 错误，响应内容: {response.text[:500]}")
                try:
                    error_detail = response.json()
                ...
                print(f"[FAIL] [Google AI 测试] 403 错误，响应内容: {response.text[:500]}")
                return {
                    "success": False,
                    "message": f"{display_name} API密钥无效或权限不足"
                }
            elif response.status_code == 503:
                print(f"[FAIL] [Google AI 测试] 503 错误，响应内容: {response.text[:500]}")
                try:
                    error_detail = response.json()
                ...
                print(f"[FAIL] [Google AI 测试] {response.status_code} 错误，响应内容: {response.text[:500]}")
                return {
                    "success": False,
    ...
    def _test_deepseek_api(self, api_key: str, display_name: str, model_name: str = None) -> dict:
        ...
        try:
            ...
            logger.info(f"[SEARCH] [DeepSeek 测试] 使用模型: {model_name}")
    ...
    def _test_dashscope_api(self, api_key: str, display_name: str, model_name: str = None) -> dict:
        ...
        try:
            ...
            logger.info(f"[SEARCH] [DashScope 测试] 使用模型: {model_name}")
    ...
    async def fetch_provider_models(self, provider_id: str) -> dict:
        """从厂家 API 获取模型列表"""
        try:
            print(f"[SEARCH] 获取厂家模型列表 - provider_id: {provider_id}")
            ...
            if not self._is_valid_api_key(api_key):
                ...
                if env_api_key:
                    ...
                    print(f"[WARN] {display_name} 未配置有效的API密钥，尝试无认证访问")
            else:
                print(f"[OK] 使用数据库配置的 {display_name} API密钥")
    ...
    def _fetch_models_from_api(self, api_key: str, base_url: str, display_name: str) -> dict:
        ...
        try:
            ...
            if api_key:
                headers["Authorization"] = f"Bearer {api_key}"
                print(f"[SEARCH] 请求 URL: {url} (with API Key)")
            else:
                print(f"[SEARCH] 请求 URL: {url} (without API Key)")

            response = requests.get(url, headers=headers, timeout=15)

            print(f"[CHART] 响应状态码: {response.status_code}")
            print(f"[CHART] 响应内容: {response.text[:500]}...")

            if response.status_code == 200:
                result = response.json()
                print(f"[CHART] 响应 JSON 结构: {list(result.keys())}")

                if "data" in result and isinstance(result["data"], list):
                    all_models = result["data"]
                    print(f"[CHART] API 返回 {len(all_models)} 个模型")

                    # 打印前几个模型的完整结构（用于调试价格字段）
                    if all_models:
                        print(f"[SEARCH] 第一个模型的完整结构:")
                        import json
                    ...
                    if anthropic_models:
                        print(f"[SEARCH] Anthropic 模型列表 ({len(anthropic_models)} 个):")
                        for m in anthropic_models[:20]:  # 只打印前 20 个
                            print(f"   - {m.get('id')}")

                    # 过滤：只保留主流大厂的常用模型
                    filtered_models = self._filter_popular_models(all_models)
                    print(f"[OK] 过滤后保留 {len(filtered_models)} 个常用模型")
                    ...
                    print(f"[FAIL] 响应格式异常，期望 'data' 字段为列表")
                    return {
                        "success": False,
            ...
            if not is_popular_provider:
                for prefix, provider in model_prefixes.items():
                    if model_id.startswith(prefix):
                        ...
                        print(f"[SEARCH] 识别模型前缀: {model_id} -> {provider}")
                        break
    ...
    def _test_openai_compatible_api(self, api_key: str, display_name: str, base_url: str = None, provider_name: str = None) -> dict:
        ...
        try:
            ...
            if provider_name == "siliconflow":
                ...
                logger.info(f"[SEARCH] 硅基流动使用测试模型: {test_model}")
            elif provider_name == "zhipu":
                # 智谱AI使用 glm-4 模型进行测试
                test_model = "glm-4"
                logger.info(f"[SEARCH] 智谱AI使用测试模型: {test_model}")
            ...
            if response.status_code == 200:
                ...
                try:
                    error_detail = response.json()
                    error_msg = error_detail.get("error", {}).get("message", f"HTTP {response.status_code}")
                    logger.error(f"[FAIL] [{display_name}] API测试失败")
