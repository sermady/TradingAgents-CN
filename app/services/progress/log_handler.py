...
class ProgressLogHandler(logging.Handler):
    ...
    def __init__(self):
        ...
        self.progress_patterns = {
            # 基础阶段
            r"验证.*股票代码|检查.*数据源": "[CLIPBOARD] 准备阶段",
            r"检查.*API.*密钥|环境.*配置": "[CONFIG] 环境检查",
            r"预估.*成本|成本.*估算": "💰 成本估算",
            r"配置.*参数|参数.*设置": "⚙️ 参数设置",
            r"初始化.*引擎|启动.*引擎": "[START] 启动引擎",

            # 分析师阶段
            r"市场分析师.*开始|开始.*市场分析|市场.*数据.*分析": "[CHART] 市场分析师正在分析",
            ...
            r"研究.*辩论|辩论.*开始|debate.*start": "[TARGET] 研究辩论进行中",
            ...
            # 风险管理阶段
            r"激进.*风险|risky.*risk": "[HOT] 激进风险评估",
            r"保守.*风险|conservative.*risk": "🛡️ 保守风险评估",
            r"中性.*风险|neutral.*risk": "⚖️ 中性风险评估",
            r"风险经理|risk.*manager": "[TARGET] 风险经理制定策略",

            # 最终阶段
            r"信号处理|signal.*process": "📡 信号处理",
            r"生成.*报告|report.*generat": "[CHART] 生成报告",
            r"分析.*完成|analysis.*complet": "[OK] 分析完成",
        }

        logger.info("[CHART] [进度日志] 日志处理器初始化完成")

    def register_tracker(self, task_id: str, tracker: RedisProgressTracker):
        """注册进度跟踪器"""
        with self._lock:
            self._trackers[task_id] = tracker
            logger.info(f"[CHART] [进度日志] 注册跟踪器: {task_id}")
    ...
    def unregister_tracker(self, task_id: str):
        ...
        with self._lock:
            if task_id in self._trackers:
                ...
                logger.info(f"[CHART] [进度日志] 注销跟踪器: {task_id}")
            ...
            for task_id, tracker in trackers_copy.items():
                try:
                    ...
                    if hasattr(tracker, 'progress_data') and tracker.progress_data.get('status') == 'running':
                        ...
                        logger.debug(f"[CHART] [进度日志] 更新进度: {task_id} -> {progress_message}")
                        break  # 只更新第一个匹配的跟踪器
                except Exception as e:
                    logger.warning(f"[CHART] [进度日志] 更新失败: {task_id} - {e}")

        except Exception as e:
            # 不要让日志处理器的错误影响主程序
            logger.error(f"[CHART] [进度日志] 日志处理错误: {e}")
...
def get_progress_log_handler() -> ProgressLogHandler:
    ...
    with _handler_lock:
        if _progress_log_handler is None:
            ...
            logger.info(f"[CHART] [进度日志] 已注册到 {len(loggers_to_monitor)} 个日志记录器")
