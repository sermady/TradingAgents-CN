...
class TushareInitService:
    ...
    async def initialize(self):
        ...
        logger.info("[OK] Tushare初始化服务准备完成")
    ...
    async def run_full_initialization(
        ...
        logger.info(f"[START] 开始Tushare数据初始化...")
        ...
        try:
            ...
            logger.info(f"[SUCCESS] Tushare数据初始化完成！耗时: {duration:.2f}秒")
            
            return self._get_initialization_summary()
            
        except Exception as e:
            logger.error(f"[FAIL] Tushare数据初始化失败: {e}")
    ...
    async def _step_initialize_financial_data(self):
        ...
        try:
            ...
            if result:
                ...
                logger.warning("[WARN] 财务数据初始化失败（可能需要更高权限）")
        except Exception as e:
    ...
    async def _step_initialize_news_data(self, historical_days: int):
        ...
        try:
            ...
            if result:
                ...
                logger.warning("[WARN] 新闻数据初始化失败（可能需要Tushare新闻权限）")
        except Exception as e:
...
async def run_tushare_full_initialization(
    ...
    try:
        ...
        logger.info(f"[OK] Tushare完整初始化完成: {result}")
        return result
    except Exception as e:
        logger.error(f"[FAIL] Tushare完整初始化失败: {e}")
        raise
