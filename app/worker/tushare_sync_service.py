...
class TushareSyncService:
    ...
    async def initialize(self):
        ...
        if not success:
            raise RuntimeError("[FAIL] Tushare连接失败，无法启动同步服务")
        ...
        logger.info("[OK] Tushare同步服务初始化完成")
    ...
    async def sync_stock_basic_info(self, force_update: bool = False, job_id: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not stock_list:
                logger.error("[FAIL] 无法获取股票列表")
                return stats
            
            stats["total_processed"] = len(stock_list)
            logger.info(f"[CHART] 获取到 {len(stock_list)} 只股票信息")

            # 2. 批量处理
            for i in range(0, len(stock_list), self.batch_size):
                # 检查是否需要退出
                if job_id and await self._should_stop(job_id):
                    logger.warning(f"[WARN] 任务 {job_id} 收到停止信号，正在退出...")
                    stats["stopped"] = True
                    break
                ...
                logger.info(f"[CHART-UP] 基础信息同步进度: {progress}/{len(stock_list)} ({progress_percent}%) "
            ...
            logger.error(f"[FAIL] 股票基础信息同步失败: {e}")
        ...
        for stock_info in batch:
            try:
                ...
                if not force_update:
                    ...
                    if existing:
                        # [HOT] existing 也可能是 Pydantic 模型，需要安全获取属性
                ...
                batch_stats["error_count"] += 1
                # [HOT] 安全获取 code（处理 Pydantic 模型和字典）
                try:
    ...
    async def sync_realtime_quotes(self, symbols: List[str] = None, force: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            # [HOT] 策略选择：少量股票切换到 AKShare，大量股票或全市场用 Tushare 批量接口
            USE_AKSHARE_THRESHOLD = 10  # 少于等于10只股票时切换到 AKShare

            if symbols and len(symbols) <= USE_AKSHARE_THRESHOLD:
                # [HOT] 自动切换到 AKShare（避免浪费 Tushare rt_k 配额，每小时只能调用2次）
                logger.info(
                    f"[INFO] 股票数量 ≤{USE_AKSHARE_THRESHOLD} 只，自动切换到 AKShare 接口"
                    f"（避免浪费 Tushare rt_k 配额，每小时只能调用2次）"
                )
                logger.info(f"[TARGET] 使用 AKShare 同步 {len(symbols)} 只股票的实时行情: {symbols}")
                ...
                if not akshare_service:
                    logger.error("[FAIL] AKShare 服务不可用，回退到 Tushare 批量接口")
                    # 回退到 Tushare 批量接口
                ...
                if symbols:
                    logger.info(f"[CHART] 使用 Tushare 批量接口同步 {len(symbols)} 只股票的实时行情（从全市场数据中筛选）")
                else:
                    logger.info("[CHART] 使用 Tushare 批量接口同步全市场实时行情...")

                logger.info("📡 调用 rt_k 批量接口获取全市场实时行情...")
                quotes_map = await self.provider.get_realtime_quotes_batch()

                if not quotes_map:
                    logger.warning("[WARN] 未获取到实时行情数据")
                    return stats

                logger.info(f"[OK] 获取到 {len(quotes_map)} 只股票的实时行情")

                # [HOT] 如果指定了股票列表，只处理这些股票
                if symbols:
                    # 过滤出指定的股票
                    ...
                    if missing_symbols:
                        logger.warning(f"[WARN] 以下股票未在实时行情中找到: {missing_symbols}")

                    quotes_map = filtered_quotes_map
                    logger.info(f"[SEARCH] 过滤后保留 {len(quotes_map)} 只指定股票的行情")

            if not quotes_map:
                logger.warning("[WARN] 未获取到任何实时行情数据")
                return stats
            ...
            if self._is_rate_limit_error(error_msg):
                ...
                logger.error(f"[FAIL] 实时行情同步失败（API限流）: {e}")
            else:
                logger.error(f"[FAIL] 实时行情同步失败: {e}")

            stats["errors"].append({"error": str(e), "context": "sync_realtime_quotes"})
            return stats

    # [HOT] 已废弃：不再使用 Tushare 单只接口（rt_k 每小时只能调用2次，太宝贵）
    ...
    #                 logger.info(f"[OK] 获取 {symbol} 实时行情成功")
    #             else:
    #                 logger.warning(f"[WARN] 未获取到 {symbol} 的实时行情")
    #         except Exception as e:
    #             logger.error(f"[FAIL] 获取 {symbol} 实时行情失败: {e}")
    #             continue
    #
    #     logger.info(f"[OK] 单只接口获取完成，成功 {len(quotes_map)}/{len(symbols)} 只")
    #     return quotes_map
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                if self._is_rate_limit_error(error_msg):
                    ...
                    logger.warning(f"[WARN] 检测到 API 限流错误: {error_msg}")
    ...
    async def _get_and_save_quotes(self, symbol: str) -> bool:
        ...
        try:
            ...
            # 检测限流错误，直接抛出让上层处理
            if self._is_rate_limit_error(error_msg):
                logger.error(f"[FAIL] 获取 {symbol} 行情失败（限流）: {e}")
                raise  # 抛出限流错误
            logger.error(f"[FAIL] 获取 {symbol} 行情失败: {e}")
            return False
    ...
    async def sync_historical_data(
        ...
        logger.info(f"[SYNC] 开始同步{period_name}历史数据...")
        ...
        try:
            ...
            if symbols is None:
                ...
                logger.info(f"[CLIPBOARD] 从 stock_basic_info 获取到 {len(symbols)} 只股票（已排除退市股票）")
            ...
            logger.info(f"[CHART] 历史数据同步: 结束日期={end_date}, 股票数量={len(symbols)}, 模式={'增量' if incremental else '全量'}")
                ...
                try:
                    ...
                    if not symbol_start_date:
                        if all_history:
                            ...
                            logger.debug(f"[DATE] {symbol}: 从 {symbol_start_date} 开始同步")
                        else:
                            symbol_start_date = (datetime.now() - timedelta(days=365)).strftime('%Y-%m-%d')

                    # 记录请求参数
                    logger.debug(
                        f"[SEARCH] {symbol}: 请求{period_name}数据 "
                    ...
                    if df is not None and not df.empty:
                        ...
                        logger.info(
                            f"[OK] {symbol}: 保存 {records_saved} 条{period_name}记录，"
                        ...
                        stock_duration = (datetime.now() - stock_start_time).total_seconds()
                        logger.warning(
                            f"[WARN] {symbol}: 无{period_name}数据 "
                    ...
                    if (i + 1) % 50 == 0 or (i + 1) == len(symbols):
                        logger.info(f"[CHART-UP] {period_name}数据同步进度: {i + 1}/{len(symbols)} ({progress_percent}%) "
                    ...
                    logger.error(
                        f"[FAIL] {symbol} {period_name}数据同步失败\n"
            ...
            logger.info(f"[OK] {period_name}数据同步完成: "
    ...
    async def _save_historical_data(self, symbol: str, df, period: str = "daily") -> int:
        ...
        try:
            ...
            logger.error(f"[FAIL] 保存{period}数据失败 {symbol}: {e}")
            return 0
        ...
        try:
            ...
            if symbol:
                ...
                if latest_date:
                    ...
                    # 如果没有上市日期，从1990年开始
                    logger.warning(f"[WARN] {symbol}: 未找到上市日期，从1990-01-01开始同步")
                    return "1990-01-01"
            ...
            logger.error(f"[FAIL] 获取最后同步日期失败 {symbol}: {e}")
            # 出错时返回30天前，确保不漏数据
            return (datetime.now() - timedelta(days=30)).strftime('%Y-%m-%d')
    ...
    async def sync_financial_data(self, symbols: List[str] = None, limit: int = 20, job_id: str = None) -> Dict[str, Any]:
        ...
        Args:
            ...
            job_id: 任务ID（用于进度跟踪）
        """
        logger.info(f"[SYNC] 开始同步财务数据 (获取最近 {limit} 期)...")
        ...
        try:
            ...
            if symbols is None:
                ...
                symbols = [doc["code"] async for doc in cursor]
                logger.info(f"[CLIPBOARD] 从 stock_basic_info 获取到 {len(symbols)} 只股票")

            stats["total_processed"] = len(symbols)
            logger.info(f"[CHART] 需要同步 {len(symbols)} 只股票财务数据")
            ...
            for i, symbol in enumerate(symbols):
                try:
                    ...
                    if financial_data:
                        ...
                        logger.warning(f"[WARN] {symbol}: 无财务数据")

                    # 进度日志和进度跟踪
                    if (i + 1) % 20 == 0:
                        progress = int((i + 1) / len(symbols) * 100)
                        logger.info(f"[CHART-UP] 财务数据同步进度: {i + 1}/{len(symbols)} ({progress}%) "
                                   f"(成功: {stats['success_count']}, 错误: {stats['error_count']})")
                        # 输出速率限制器统计
                        limiter_stats = self.rate_limiter.get_stats()
                        logger.info(f"   速率限制: {limiter_stats['current_calls']}/{limiter_stats['max_calls']}次")
                        ...
                        if job_id:
                            ...
                            try:
                                ...
                                # 任务被取消，记录并退出
                                logger.warning(f"[WARN] 财务数据同步任务被用户取消 (已处理 {i + 1}/{len(symbols)})")
                                stats["end_time"] = datetime.utcnow()
                                stats["duration"] = (stats["end_time"] - stats["start_time"]).total_seconds()
                    ...
                    logger.error(f"[FAIL] {symbol} 财务数据同步失败: {e}")

            # 完成统计
            stats["end_time"] = datetime.utcnow()
            stats["duration"] = (stats["end_time"] - stats["start_time"]).total_seconds()

            logger.info(f"[OK] 财务数据同步完成: "
            ...
            logger.error(f"[FAIL] 财务数据同步失败: {e}")
    ...
    async def _save_financial_data(self, symbol: str, financial_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(f"[FAIL] 保存 {symbol} 财务数据失败: {e}")
            return False
    ...
    async def sync_news_data(
        ...
        try:
            ...
            if not symbols:
                logger.warning("[WARN] 没有找到需要同步新闻的股票")
                return stats

            stats["total_processed"] = len(symbols)
            logger.info(f"[CHART] 需要同步 {len(symbols)} 只股票的新闻")
            ...
            for i in range(0, len(symbols), self.batch_size):
                ...
                logger.info(f"[CHART-UP] 新闻同步进度: {progress}/{len(symbols)} ({progress_percent}%) "
            ...
            logger.error(f"[FAIL] 新闻数据同步失败: {e}")
        ...
        for symbol in batch:
            try:
                ...
                if news_data:
                    ...
                    logger.debug(f"[OK] {symbol} 新闻同步成功: {saved_count}条")
                else:
                    logger.debug(f"[WARN] {symbol} 未获取到新闻数据")
                ...
                logger.error(f"[FAIL] {symbol} 新闻同步失败: {e}")
    ...
    async def _should_stop(self, job_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"[FAIL] 检查任务停止标记失败: {e}")
            return False
    ...
    async def _update_progress(self, job_id: str, progress: int, message: str):
        ...
        try:
            ...
            logger.info(f"[CHART] [进度更新] 开始更新任务 {job_id} 进度: {progress}% - {message}")
            ...
            if not execution:
                logger.warning(f"[WARN] 未找到任务 {job_id} 的执行记录")
                sync_client.close()
                return

            logger.info(f"[CHART] [进度更新] 找到执行记录: _id={execution['_id']}, 当前进度={execution.get('progress', 0)}%")
            ...
            logger.info(f"[CHART] [进度更新] 更新结果: matched={result.matched_count}, modified={result.modified_count}")

            sync_client.close()
            logger.info(f"[OK] 任务 {job_id} 进度更新成功: {progress}% - {message}")

        except Exception as e:
            if "TaskCancelledException" in str(type(e).__name__):
                raise
            logger.error(f"[FAIL] 更新任务进度失败: {e}", exc_info=True)
...
async def run_tushare_basic_info_sync(force_update: bool = False):
    ...
    try:
        ...
        logger.info(f"[OK] Tushare基础信息同步完成: {result}")
        return result
    except Exception as e:
        logger.error(f"[FAIL] Tushare基础信息同步失败: {e}")
        raise
...
async def run_tushare_quotes_sync(force: bool = False):
    ...
    try:
        ...
        logger.info(f"[OK] Tushare行情同步完成: {result}")
        return result
    except Exception as e:
        logger.error(f"[FAIL] Tushare行情同步失败: {e}")
        raise


async def run_tushare_historical_sync(incremental: bool = True):
    """APScheduler任务：同步历史数据"""
    logger.info(f"[START] [APScheduler] 开始执行 Tushare 历史数据同步任务 (incremental={incremental})")
    try:
        service = await get_tushare_sync_service()
        logger.info(f"[OK] [APScheduler] Tushare 同步服务已初始化")
        result = await service.sync_historical_data(incremental=incremental, job_id="tushare_historical_sync")
        logger.info(f"[OK] [APScheduler] Tushare历史数据同步完成: {result}")
        return result
    except Exception as e:
        logger.error(f"[FAIL] [APScheduler] Tushare历史数据同步失败: {e}")
        import traceback
...
async def run_tushare_financial_sync():
    ...
    try:
        ...
        logger.info(f"[OK] Tushare财务数据同步完成: {result}")
        return result
    except Exception as e:
        logger.error(f"[FAIL] Tushare财务数据同步失败: {e}")
        raise
...
async def run_tushare_status_check():
    ...
    try:
        ...
        logger.info(f"[OK] Tushare状态检查完成: {result}")
        return result
    except Exception as e:
        logger.error(f"[FAIL] Tushare状态检查失败: {e}")
        return {"error": str(e)}
...
async def run_tushare_news_sync(hours_back: int = 24, max_news_per_stock: int = 20):
    ...
    try:
        ...
        logger.info(f"[OK] Tushare新闻数据同步完成: {result}")
        return result
    except Exception as e:
        logger.error(f"[FAIL] Tushare新闻数据同步失败: {e}")
        raise
